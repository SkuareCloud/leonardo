...
export function ActivationDialog({ network, avatarId, onActivationComplete }: ActivationDialogProps) {
  ...
  const pollStatus = async () => {
    try {
      ...
      if (status === "SUCCESS" || status === "LOGIN_VERIFICATION_FAILED" || status === "FAILED") {
        ...
        logger.info(`Continuing to poll in 2 seconds for ${avatarId}, current status: ${status}`)
  ...
  const handleActivation = async () => {
    try {
      ...
      // Start activation
      await new ServiceBrowserClient().activate(avatarId, true, shouldOverride, sessionData || null)
  ...
  const handleOtpSubmit = async () => {
    ...
    try {
      ...
      await new ServiceBrowserClient().submitCredentials(avatarId, otp.trim(), password.trim())
    ...
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      ...
      <CustomDialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Activate the {network} account for this avatar. This will start the activation process.
          </DialogDescription>
        </DialogHeader>
        {!isPolling && !isWaitingForOtp ? (
          <div className="py-4">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              This process may take a few minutes and may require manual intervention for verification steps.
            </p>
            <div className="space-y-4">
                ...
                <Select
                  ...
                  onValueChange={value => {
                    ...
                    if (sessionDataRef.current) {
                      sessionDataRef.current.style.display = value === "session" ? "block" : "none"
                    }
                  }}
                  onOpenChange={open => {
              ...
              <div className="flex items-center space-x-2">
                <Checkbox 
                  ...
                  onCheckedChange={(checked) => setShouldOverride(checked === true)}
                />
          ...
          <div className="py-4">
            <div className="flex flex-col gap-4">
              {activationError ? (
                ...
                <>
                  <div className="text-center">
                    <div className="text-lg font-semibold mb-2">OTP Required</div>
                    <div className="text-sm text-muted-foreground mb-4">
                      Please enter the OTP code sent to your device and optionally your password.
                    </div>
                  </div>
                  <div className="space-y-4">
                    <div className="flex flex-col gap-2">
                ...
                <>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-lg font-semibold">
                      {activationStatus === "SUCCESS" || activationStatus === "LOGIN_VERIFICATION_FAILED" || activationStatus === "FAILED" 
                        ? "Activation Complete" 
                  ...
                  <div className="text-center">
                    ...
                    <div className="text-sm text-muted-foreground capitalize flex items-center justify-center gap-2">
                      ...
                      {isPolling && (
                        <div className="w-2 h-2 bg-blue-500 rounded-full animate-pulse" title="Updating status..."></div>
                      )}
                    </div>
                    {lastUpdated && (
                  ...
                  {activationStatus === "SUCCESS" && (
                    <div className="text-center">
                      ...
                      <div className="text-sm text-muted-foreground">
                        The {network} account has been successfully activated and is ready to use.
                      </div>
                    </div>
                  )}
                  {activationStatus === "LOGIN_VERIFICATION_FAILED" && (
                    <div className="text-center">
                      <div className="text-red-500 text-lg font-semibold mb-2">❌ Login Verification Failed</div>
                      <div className="text-sm text-muted-foreground">
                        The login verification process failed. This could be due to incorrect credentials or security measures. Please try again.
                      </div>
                    </div>
                  )}
                  {activationStatus === "FAILED" && (
                    <div className="text-center">
                      <div className="text-red-500 text-lg font-semibold mb-2">❌ Activation Failed</div>
                      <div className="text-sm text-muted-foreground">
                        The activation process failed. Please check your settings and try again.
                      </div>
                    </div>
                  )}
                  {/* Show intermediate status messages */}
                  {activationStatus && !["SUCCESS", "LOGIN_VERIFICATION_FAILED", "FAILED"].includes(activationStatus) && (
                    <div className="text-center">
                      <div className="text-sm text-muted-foreground">
                        {activationStatus === "CHECKING_PROFILE" && "Checking profile status..."}
                        {activationStatus === "STARTED" && "Starting browser session..."}
                        {activationStatus === "ALREADY_LOGGED_IN" && "Profile already logged in..."}
                        {activationStatus === "ENTERING_OTP" && "Processing OTP..."}
                        {activationStatus === "CHECKING_IF_WAITING_FOR_PASSWORD" && "Checking if password is required..."}
                        {activationStatus === "WAITING_FOR_PASSWORD" && "Waiting for password input..."}
                        {activationStatus === "ENTERING_PASSWORD" && "Processing password..."}
                        {activationStatus === "WAITING_10_SECONDS" && "Waiting for verification..."}
                        {activationStatus === "VERIFYING_LOGIN" && "Verifying login status..."}
                        {activationStatus === "SKIPPED" && "Step skipped..."}
                      </div>
                    </div>
                  )}
                </>
        ...
        <DialogFooter>
          {!isPolling && !isWaitingForOtp ? (
            ...
            <>
              ...
              <Button onClick={handleOtpSubmit} disabled={isSubmittingOtp || !otp.trim()}>
                {isSubmittingOtp ? (
                  <>
