            ...
            const handleUpdateStatus = async (enabled: boolean) => {
                try {
                    const response = await fetch(
                        `/api/proxies/${proxy.id}/status?enabled=${enabled}`,
                        {
                            method: "PUT",
                        },
                    )
                    ...
                    if (response.ok) {
                        ...
                        const data = await response.json()
                        throw new Error(
                            data.error || `Failed to ${enabled ? "enable" : "disable"} proxy`,
                        )
                    }
                } catch (error) {
                    ...
                    {proxy.status_is_success ? (
                        <AlertDialog>
                            ...
                            <AlertDialogContent>
                                ...
                                <AlertDialogFooter>
                                    ...
                                    <AlertDialogAction onClick={() => handleUpdateStatus(false)}>
                                        Disable Proxy
                                    </AlertDialogAction>
                                </AlertDialogFooter>
                            </AlertDialogContent>
                        ...
                        <AlertDialog>
                            ...
                            <AlertDialogContent>
                                ...
                                <AlertDialogFooter>
                                    <AlertDialogCancel>Cancel</AlertDialogCancel>
                                    <AlertDialogAction onClick={() => handleUpdateStatus(true)}>
                                        Enable Proxy
                                    </AlertDialogAction>
                                </AlertDialogFooter>
                            </AlertDialogContent>
