...
const missionColumns: ColumnDef<MissionWithExposureAndStats>[] = [
    ...
    },
    {
        ...
        cell: ({ row }) => {
            const mission = row.original
            return (
                <span className="w-[200px]">{mission.mission.description || "No description"}</span>
            )
        },
    },
    {
        ...
        filterFn: (row, id, value) => {
            return (
                row.original.mission.status_code?.toLowerCase().includes(value.toLowerCase()) ||
                false
            )
        },
        sortingFn: (rowA, rowB) => {
    ...
    },
    {
        ...
        cell: ({ row }) => {
            ...
            return (
                <span>
                    {(
                        ((mission.mission.success_scenarios || 0) /
                            (mission.mission.scenarios_count || 1)) *
                        100
                    ).toPrecision(3)}
                    %
                </span>
    ...
    },
    {
        ...
        cell: ({ row }) => {
            ...
            return (
                <span>
                    {humanizeNumber(actualReach)} / {humanizeNumber(potentialReach)} (
...
export function MissionsList({ data }: { data: MissionWithExposureAndStats[] }) {
    ...
    const refreshMissions = async () => {
        const missions =
            await new ServiceBrowserClient().getOrchestratorMissionsWithExposureAndStats()
                ...
                try {
                    ...
                    if (scenariosCount > 0) {
                        const anyRunning = scenarios.some((s) =>
                            ["running", "in_process", "pending", "scheduled", "planned"].includes(
    ...
    return (
        <div className="flex w-full flex-col">
            <div className="flex max-w-[70vw] flex-col gap-4 md:max-w-[1800px]">
                <DataTable
                        ...
                        return (
                            <div className="flex h-full flex-row items-center gap-6">
                                <Combobox
                                    ...
                                    onValueChange={(value) =>
                                        table.getColumn("mission_type")?.setFilterValue(value)
                                    }
                                    className="max-w-sm"
                                />
                                <Combobox
                                    ...
                                    onValueChange={(value) =>
                                        table.getColumn("status_code")?.setFilterValue(value)
                                    }
                                    className="max-w-sm"
                                />
                                {table.getFilteredSelectedRowModel().rows.length > 0 && (
                                    <>
                                        <Separator
                                            ...
                                            className="mx-4 max-h-6 border-1"
                                        />
                                        <Button
                                            variant="destructive"
                                            size="sm"
                                            onClick={async () => {
                                                const selectedMissions = table
                                                    .getFilteredSelectedRowModel()
                                                    .rows.map((row) => row.original)
                                                try {
                                                    for (const mission of selectedMissions) {
                                                        await new ServiceBrowserClient().deleteMission(
                                                            mission.mission.id,
                                                        )
                                                    }
                                                    ...
                                                    toast.error(
                                                        "Failed to delete at least one mission: " +
                                                            error,
                                                    )
                                                    logger.error(error)
                                                } finally {
