...
export default async function Page({ params }: { params: { id: string } }) {
  ...
  const chatMemberAvatars = allAvatars.filter(avatar => 
    characterIds.includes(avatar.id)
  )
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="flex flex-col pt-6">
        <ChatView chat={chat} chatCategories={chatCategories} allCategories={allCategories} avatars={chatMemberAvatars} />
      </div>
    </div>
  )
}
