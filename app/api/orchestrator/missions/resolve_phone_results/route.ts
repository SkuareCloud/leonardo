...
export async function POST(request: NextRequest) {
  ...
  const categories: string[] | undefined = charactersCategoriesRaw ? JSON.parse(charactersCategoriesRaw) : undefined
  ...
  const response = await createResolvePhoneMissionMissionsResolvePhoneResultsPost({
    ...
    query: {
      ...
      max_phones_per_scenario: maxPhonesPerScenario ? Number(maxPhonesPerScenario) : undefined,
